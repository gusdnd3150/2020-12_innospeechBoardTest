...
<body>
	...
	<div class="container" align="center" style="padding-top: 10%">
		...
		<div class="tabList" style="border-style: groove; padding: 2%;"
			align="left">
			<form name="tapFormArea" method="get" action="/Test/searchBoard.do">
				<div class="searchPanel">
					<input type="text" style="padding: 0.6%;" name="search_text" value="" placeholder="검색어를 입력해주세요." /> 
						<select name="searchType" style="padding: 0.8%;">
						<option value="t">제목</option>
						<option value="c">내용</option>
						<option value="n">작성자</option>
					</select>
					<button type="button" onclick="searchBoard(event);">
			...
			<div class="col-lg-12">
				...
				<div style="display: block; text-align: center;">
					<c:if test="${paging.startPage != 1 }">
						<a
							href="/Test/mainBoard.do?nowPage=${paging.startPage - 1 }&cntPerPage=${paging.cntPerPage}">&lt;</a>
					</c:if>
					<c:forEach begin="${paging.startPage }" end="${paging.endPage }"
						...
						<c:choose>
							...
							<c:when test="${p != paging.nowPage }">
								<a
									href="/Test/mainBoard.do?nowPage=${p }&cntPerPage=${paging.cntPerPage}">${p }</a>
							</c:when>
						</c:choose>
					</c:forEach>
					<c:if test="${paging.endPage != paging.lastPage}">
						<a
							href="/Test/mainBoard.do?nowPage=${paging.endPage+1 }&cntPerPage=${paging.cntPerPage}">&gt;</a>
					</c:if>
				</div>
